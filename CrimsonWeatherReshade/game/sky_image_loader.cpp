...
namespace {
    ...
    bool LoadDdsFile(const std::string& path, SkyImageData& outImage, char* status, size_t statusSize) {
        ...
        if ((pfFlags & 4) == 0) {
            ...
            Log("[sky-image] DDS is not FourCC compressed pfFlags=0x%X path=%s\n", pfFlags, path.c_str());
            return false;
        }
        ...
        if (fourcc == ReadU32(reinterpret_cast<const uint8_t*>("DXT1"))) {
            ...
            Log("[sky-image] DDS unsupported fourcc=%s path=%s\n", cc, path.c_str());
            return false;
        }
    ...
    bool LoadPngFile(const std::string& path, SkyImageData& outImage, char* status, size_t statusSize) {
        ...
        Log("[sky-image] loaded PNG %s %ux%u fmt=%s(%u) bytes=%zu\n",
            path.c_str(),
            outImage.width,
            outImage.height,
            FormatName(outImage.format),
