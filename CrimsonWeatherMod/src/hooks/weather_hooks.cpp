...
void UpdateControllerNavInput() {
    g_padCmd = {};
    if (!g_imguiReady || !g_menuOpen) return;
    ULONGLONG now = GetTickCount64();
    if (now < g_uiInputSuppressUntil.load()) return;

    WORD buttons = 0;
    OverlayPadSource source = OverlayPadSource::None;

    XINPUT_STATE state{};
    if (g_pOrigXInputGetState && g_pOrigXInputGetState(0, &state) == ERROR_SUCCESS) {
        buttons = state.Gamepad.wButtons;
        source = OverlayPadSource::XInput;
    } else if (g_dualSenseLastInputMs != 0 && ((now - g_dualSenseLastInputMs) <= 1500ULL || g_dualSenseButtons != 0)) {
        buttons = g_dualSenseButtons;
        source = OverlayPadSource::GameInput;
    } else {
        g_padUiPollPrevButtons = 0;
        g_padUiPollSource = OverlayPadSource::None;
        return;
    }

    if (g_padUiPollSource != source) {
        g_padUiPollSource = source;
        g_padUiPollPrevButtons = 0;
    ...
    auto isDown = [&](WORD b) -> bool { return (buttons & b) != 0; };
    auto wasDown = [&](WORD b) -> bool { return (g_padUiPollPrevButtons & b) != 0; };
    auto pressed = [&](WORD b) -> bool { return isDown(b) && !wasDown(b); };
    auto HoldScale = [&](int idx, WORD b) -> float {
        if (!isDown(b) || g_padHoldStartMs[idx] == 0) return 1.0f;
        ULONGLONG held = now - g_padHoldStartMs[idx];
        if (held >= 2400) return 6.0f;
        if (held >= 1600) return 4.0f;
        if (held >= 900)  return 2.5f;
        if (held >= 450)  return 1.5f;
        return 1.0f;
    };
    auto repeat = [&](WORD b, int idx) -> bool {
        const bool down = isDown(b);
        const bool was = wasDown(b);
        if (!down) {
            g_padHoldNextMs[idx] = 0;
            g_padHoldStartMs[idx] = 0;
            return false;
        }
        if (down && !was) {
            g_padHoldStartMs[idx] = now;
            g_padHoldNextMs[idx] = now + 220;
            return true;
        }
        if (g_padHoldNextMs[idx] != 0 && now >= g_padHoldNextMs[idx]) {
            ULONGLONG held = g_padHoldStartMs[idx] ? (now - g_padHoldStartMs[idx]) : 0;
            ULONGLONG stepMs = 70;
            if (held >= 2000) stepMs = 18;
            else if (held >= 1200) stepMs = 28;
            else if (held >= 700) stepMs = 42;
            g_padHoldNextMs[idx] = now + stepMs;
            return true;
        }
        return false;
    };

    const bool tabChanged = pressed(XINPUT_GAMEPAD_LEFT_SHOULDER) || pressed(XINPUT_GAMEPAD_RIGHT_SHOULDER);
    if (tabChanged) {
        constexpr int kTabCount = 6;
        int idx = g_uiTabIndex + (pressed(XINPUT_GAMEPAD_RIGHT_SHOULDER) ? 1 : -1);
        while (idx < 0) idx += kTabCount;
        while (idx >= kTabCount) idx -= kTabCount;
        g_uiTabIndex = idx;
        g_uiTabSelectRequested = true;
    }

    g_padCmd.up = repeat(XINPUT_GAMEPAD_DPAD_UP, 0);
    g_padCmd.down = repeat(XINPUT_GAMEPAD_DPAD_DOWN, 1);
    g_padCmd.left = repeat(XINPUT_GAMEPAD_DPAD_LEFT, 2);
    g_padCmd.right = repeat(XINPUT_GAMEPAD_DPAD_RIGHT, 3);
    g_padCmd.a = pressed(XINPUT_GAMEPAD_A);
    g_padCmd.leftScale = HoldScale(2, XINPUT_GAMEPAD_DPAD_LEFT);
    g_padCmd.rightScale = HoldScale(3, XINPUT_GAMEPAD_DPAD_RIGHT);
    if (tabChanged || g_padCmd.up || g_padCmd.down || g_padCmd.left || g_padCmd.right || g_padCmd.a) {
        g_uiControllerMode.store(true);
    }
    g_padUiPollPrevButtons = buttons;
}
...
static void ApplyClearWeatherParams(long long self, const ResolvedEnv& env) {
    ...
    if (env.cloudNode) {
        ...
        At<float>(env.cloudNode, CN::CLOUD_TOP)    = 0.0f;
        At<float>(env.cloudNode, CN::CLOUD_THICK)  = 0.0f;
        At<float>(env.cloudNode, CN::CLOUD_BASE)   = 0.0f;
        At<float>(env.cloudNode, CN::CLOUD_SHAPE_A)= 0.0f;
        At<float>(env.cloudNode, CN::CLOUD_SHAPE_B)= 0.0f;
        At<float>(env.cloudNode, CN::CLOUD_SHAPE_C)= 0.0f;
    }
