...
bool PresetDataEquals(const WeatherPresetData& a, const WeatherPresetData& b) {
    return a.forceClearSky == b.forceClearSky &&
        ...
        a.forceCloudsEnabled == b.forceCloudsEnabled &&
        FloatNearlyEqual(a.forceCloudsPercent, b.forceCloudsPercent) &&
...
std::vector<PresetWriteEntry> BuildPresetWriteEntries(const WeatherPresetData& data) {
    ...
    const auto push = [&](const char* section, const char* key, std::string value, std::vector<std::string> aliases = {}) {
        entries.push_back(PresetWriteEntry{ section, key, std::move(value), std::move(aliases) });
    };
    ...
    push("Cloud", "ForceCloudsEnabled", FormatPresetBool(data.forceCloudsEnabled));
    push("Cloud", "ForceClouds", FormatPresetFloat(min(100.0f, max(0.0f, data.forceCloudsPercent))));
    ...
    push("Cloud", "PuddleScaleEnabled", FormatPresetBool(data.puddleScaleEnabled));
    push("Cloud", "PuddleScale", FormatPresetFloat(Clamp01(data.puddleScale)));
...
bool PresetDocKeyMatches(const PresetDocLine& line, const PresetWriteEntry& entry) {
    ...
    if (!EqualsNoCase(line.section, entry.section)) return false;
...
bool LoadPresetFileInternal(const char* path, WeatherPresetData& outData) {
    ...
    data.forceCloudsPercent = min(100.0f, max(0.0f, data.forceCloudsPercent));
    if (!forceCloudsEnabledSeen) data.forceCloudsEnabled = false;
...
bool WritePresetFileInternal(const char* path, const WeatherPresetData& data) {
    PresetDocument doc{};
    if (!ParsePresetDocument(path, doc)) {
        doc = {};
        doc.headerSeen = true;
        doc.lines.push_back(MakeHeaderPresetLine());
    }

    MergePresetDocumentWithData(doc, data);
    const std::string serialized = SerializePresetDocument(doc);
